...
const EventSignup: React.FC = () => {
  ...
  const createEventSignup = async (paymentType: 'one_time' | 'subscription', amount: number) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/${paymentType === 'one_time' ? 'create-checkout-session' : 'create-subscription'}?event_signup_id=${eventSignup.id}`, {
        method: 'POST',
        headers: {
